...
async function withClient<T>(userId: number, fn: (client: Client, settings: FtpSettings) => Promise<T>): Promise<T> {
...
export async function testFtpConnectionWithSettings(settings: FtpSettings): Promise<{ ok: boolean; error?: string }> {
    const client = await createClient(settings).catch((err: any) => { throw err; });
    try {
...
export async function downloadToStream(userId: number, remotePath: string, dest: Writable): Promise<void> {
    await withClient(userId, (client) => client.downloadTo(dest, remotePath));
}

/** Upload depuis un stream vers un chemin distant. */
export async function uploadFromStream(userId: number, source: Readable, remotePath: string): Promise<void> {
...
export async function getStorageUsage(userId: number): Promise<{ used: number; limit: number | null }> {
    const settings = getFtpSettings(userId);
    ...
    return {
        used,
        limit: settings.storageLimit !== null ? (settings.storageLimit ?? 0) * 1024 * 1024 * 1024 : null,
    };
}