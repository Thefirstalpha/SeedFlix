...
router.post('/ftp/configure', async (req, res) => {
    ...
    const setting: FtpSettings = {
        ...
        password: req.body?.password !== undefined ? String(req.body?.password || '').trim() : undefined,
    ...
    if (!test.ok) {
        res.status(400).json({ ok: false, error: test.error || 'Connexion FTP échouée, configuration non sauvegardée.' });
        return;
    }
...
router.post('/ftp/mkdir', async (req, res) => {
    ...
    if (!path) { res.status(400).json({ error: 'path requis' }); return; }
    try {
...
router.post('/ftp/delete-batch', async (req, res) => {
    ...
    if (!Array.isArray(items) || items.length === 0) {
        res.status(400).json({ error: 'items requis (array)' }); return;
    }
    try {
...
router.post('/ftp/rename', async (req, res) => {
    ...
    if (!oldPath || !newPath) { res.status(400).json({ error: 'oldPath et newPath requis' }); return; }
    try {
...
router.post('/ftp/move', async (req, res) => {
    ...
    if (!Array.isArray(paths) || paths.length === 0 || !destinationDir) {
        res.status(400).json({ error: 'paths et destinationDir requis' }); return;
    }
    try {
