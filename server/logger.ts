...
const WHITELISTED_HTTP_PATHS = new Set(['GET /api/user', 'GET /api/auth/me', 'GET /api/admin/logs', 'GET /api/health']);
...
export class Logger {
  ...
  static http(req: Request, res: Response, duration: number) {
    if (
      WHITELISTED_HTTP_PATHS.has(`${req.method} ${req.originalUrl}`) &&
      res.statusCode < 400
    ) {
      return;
    }
