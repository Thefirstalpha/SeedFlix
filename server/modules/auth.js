...
function verifyPassword(password, salt, expectedHash) {
  ...
  return calculatedHash.length === storedHash.length && timingSafeEqual(calculatedHash, storedHash);
}
...
export async function initializeAuthStores() {
  ...
  mutateJsonStore(SESSIONS_STORE_KEY, [], (sessions) => (Array.isArray(sessions) ? sessions : []));
...
function isTorrentConfigured(user) {
  ...
  return Boolean(
    String(torrentSettings.username || '').trim() && String(torrentSettings.password || '').trim(),
  );
}
...
function isIndexerConfigured(user) {
  ...
  return Boolean(
    String(indexerSettings.url || '').trim() && String(indexerSettings.token || '').trim(),
  );
}
...
export function registerAuthRoutes(app) {
    ...
    withAuth(async (req, res, auth) => {
      try {
        ...
        const changeResult = runInTransaction((tx) => {
          ...
          if (
            !foundUser ||
            !verifyPassword(currentPassword, foundUser.passwordSalt, foundUser.passwordHash)
          ) {
            return { ok: false };
          }
  ...
  app.get(
    '/api/settings/database/:namespace',
    withAdmin(async (req, res) => {
      const t = getTranslator(req);
      try {
        ...
        const resetResult = runInTransaction((tx) => {
          ...
          const nextUsers = safeUsers.map((u) => {
            ...
            return {
              ...
              shouldChangePassword: !u.firstLoginPending,
            };
          });
