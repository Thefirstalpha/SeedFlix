...
export const authentication = (req: Request, res: Response, next: NextFunction) => {
  ...
  const sessions = runInTransaction(() => {
    ...
    return db
      .prepare('SELECT user_id, created_at FROM auth_sessions WHERE token = ?')
      .get(sessionToken);
  });
  if (!sessions) {
    return res.status(401).json({ error: messages.auth.authRequired });
...
export function createAuth(username: string, password: string | null = null) {
  ...
  if (result) throw new ErrorCode(messages.auth.usernameAlreadyExists);
  const id = db
    .prepare('INSERT INTO auth_users (username, salt, hash) VALUES (?, ?, ?)')
    .run(username, salt, hash).lastInsertRowid;
