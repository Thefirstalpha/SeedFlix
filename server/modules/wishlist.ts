...
export async function getWishlist(userId: number): Promise<WishListItem[]> {
  ...
  for (const item of whishlist) {
    if (typeof item === 'object' && item !== null) {
      ...
      let seasons: Record<
        number,
        { season_number: number; all_episodes: boolean; episodes: number[] }
      > = {};
...
export async function addToWishlist(
  ...
  await runInTransaction(async ({ writeStore }) => {
    ...
    let existingItem: WishListItem | undefined = whishlist.find(
      (item: WishListItem) => item.tmdb === tmdbId && item.type === type,
    );
    if (existingItem) {
      if (type === 'series') {
          ...
          if (episodeNumber !== undefined) {
            if (existingItem.seasons[seasonNumber] === undefined) {
              ...
              if (
                ...
                !existingItem.seasons[seasonNumber].episodes.includes(episodeNumber)
              ) {
...
export async function deleteWishlist(
  ...
  await runInTransaction(async ({ writeStore }) => {
    ...
    if (type === 'series' && (seasonNumber !== undefined || episodeNumber !== undefined)) {
      const item = whishlist.find(
        (item: WishListItem) => item.tmdb === tmdbId && item.type === 'series',
      );
      if (item) {
        if (seasonNumber !== undefined) {
          if (episodeNumber !== undefined) {
