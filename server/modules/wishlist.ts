    ...
    await runInTransaction(async ({ writeStore }) => {
        ...
        if (existingItem) {
            ...
            let item: WishListItem = {
                ...
                genre: type === 'movie' ? (results.genres && results.genres.length > 0 ? results.genres[0].name : '') : (results.genres && results.genres.length > 0 ? results.genres[0].name : ''),
                rating: type === 'movie' ? results.vote_average : results.vote_average,
